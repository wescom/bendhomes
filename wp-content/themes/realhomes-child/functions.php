...
if (!function_exists('inspiry_enqueue_child_styles')) {
    function inspiry_enqueue_child_styles(){
        if ( !is_admin() ) {
			...
			wp_enqueue_script( 'touchswipe', get_stylesheet_directory_uri().'/js/jquery.touchwipe.min.js', array( 'jquery' ), '', true );
			wp_enqueue_script( 'sidr', get_stylesheet_directory_uri().'/js/jquery.sidr.js', array( 'jquery' ), '', true );
